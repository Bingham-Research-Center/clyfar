...
def run_inference(forecast_data: dict, percentiles, bogus_add=None):
    ...
    # First get the datetimes that is used by the snow time series.
    # TODO - make sure there are NaNs for missing data such as time 0 in solar
    # datetimes = forecast_data[member_names[0]]['snow'].index
    # output_df = pd.DataFrame(index=datetimes)
