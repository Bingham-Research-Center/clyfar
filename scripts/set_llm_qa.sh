...
#   source scripts/set_llm_qa.sh --ffion-version 1.1.2
#   source scripts/set_llm_qa.sh --ffion-manifest templates/llm/ffion/ffion_v1.1.3.json
...
# through the Ffion bundle for the selected FFION_VERSION.
...
Usage:
  source scripts/set_llm_qa.sh
  source scripts/set_llm_qa.sh off
  source scripts/set_llm_qa.sh --ffion-version VERSION
  source scripts/set_llm_qa.sh --ffion-manifest PATH
...
if [[ -n "$QA_FILE_OVERRIDE" ]]; then
    ...
    resolve_cmd=(python3 "$REPO_ROOT/scripts/resolve_ffion_bundle.py" --field qa_file)
    if [[ -n "$FFION_VERSION_OVERRIDE" ]]; then
        resolve_cmd+=(--ffion-version "$FFION_VERSION_OVERRIDE")
    fi
    if [[ -n "$FFION_MANIFEST" ]]; then
        resolve_cmd+=(--ffion-manifest "$FFION_MANIFEST")
    fi
    QA_FILE="$("${resolve_cmd[@]}")"
fi
