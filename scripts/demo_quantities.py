...
def main() -> None:
    ...
    ax.set_title(f"Daily p50 distribution across members · {init}")
    fig.tight_layout()
    fig.savefig(out_dir / f"boxplot_p50_{init}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_ylabel("Ozone (ppb)")
    ax.set_title(f"Ensemble p50 fan across members · {init}")
    ax.legend(loc="upper right", fontsize=8)
    fig.tight_layout()
    fig.savefig(out_dir / f"fan_p50_{init}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_ylabel("Count")
    ax.set_title(f"Distribution of peak p90 across members · {init}")
    fig.tight_layout()
    fig.savefig(out_dir / f"hist_max_p90_{init}.png", bbox_inches="tight")
    plt.close(fig)
